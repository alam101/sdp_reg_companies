...
export class WeightGuagePage implements AfterViewInit {
  ...
  close() {
    ...
    this.router.navigate(["new-diet", { refresh: new Date().getTime() }]);
  }
  ionViewWillEnter() {
  ...
  weightGraph() {
    ...
    this.appService.getWeightGraph().then((res) => {
      ...
      let currentWeight =
        profile.demographic.weight.unit == "kg"
          ? res["currentWeight"]
          : res["currentWeight"] / this.poundValue;
      this.weightGraphData.currentWeight = currentWeight;
      this.weightGraphData.dailyWeightArray =
        profile.demographic.weight.unit == "kg"
          ? this.weightGraphData.dailyWeightArray
          : this.weightGraphData.dailyWeightArray / this.poundValue;
      ...
      this.weightDaily = parseFloat(this.weightGraphData.currentWeight).toFixed(
        1,
      );
      this.targetWeightDaily = (
        profile.demographic.weight.unit == "pound"
          ? this.weightGraphData.suggstedWeight / this.poundValue
          : this.weightGraphData.suggstedWeight
      ).toFixed(1);
      this.currentBMI.value = (
        parseFloat(this.currrentValueInKG) /
        (heightInMeter * heightInMeter)
      ).toFixed(1);
      this.targetBMI.value = (
        parseFloat(this.weightGraphData.suggstedWeight) /
        (heightInMeter * heightInMeter)
      ).toFixed(1);
    });
  }
  ...
  minusTargetWeight() {
    this.targetWeightDaily = (parseFloat(this.targetWeightDaily) - 1).toFixed(
      1,
    );
  ...
  saveTarget() {
    this.storage.get("profileData").then((val) => {
      ...
      //  BMI =Kg/m2
      let targetWeightDaily =
        profile.demographic.weight.unit == "pound"
          ? this.targetWeightDaily * this.poundValue
          : this.targetWeightDaily;
      let heightInMeter = 0;
      if (profile.demographic.height.unit == "cm") {
        heightInMeter = profile.demographic.height.value * 0.01;
      ...
      if (
        parseFloat(targetWeightDaily) / (heightInMeter * heightInMeter) <
        22
      ) {
  ...
  saveCurrent() {
    this.storage.get("profileData").then((val) => {
      ...
      let weightDaily =
        profile.demographic.weight.unit == "pound"
          ? this.weightDaily * this.poundValue
          : this.weightDaily;
      this.appService
        .updateCurrentWeight({
  ...
  getHealthData() {
    // this.storage.get("profileData").then(val => {
    this.profileData = this.utilities.parseJSON(
      localStorage.getItem("profileData"),
    );
    this.appService
      .getHealthData(this.profileData.profile.email)
      .then((res: any) => {
        ...
        if (res && res.length) {
          ...
          res.forEach((element) => {
            element["formatedDate"] = moment(
              new Date(element.updateDate),
            ).format("MMM D, YYYY");
            // this.profileData.demographic.weight.value - element?.healthData?.weightKg
            if (
              element?.healthData?.weightKg &&
              this.profileData.demographic.weight.value >
                element?.healthData?.weightKg
            ) {
              element["displayWeight"] = element?.healthData?.weightKg
                ? "+ " +
                  (this.profileData.demographic.weight.value -
                    element?.healthData?.weightKg)
                : null;
          ...
          let obj = {
            formatedDate: moment(
              new Date(this.profileData.profile.updatedDateTime),
            ).format("MMM D, YYYY"),
