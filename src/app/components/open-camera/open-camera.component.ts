  ...
  implements AfterViewInit, OnChanges, OnDestroy
{
  ...
  async openNutritionModelBarCode(previewUrl, foodName, foodDetail) {
    console.log(
      "previewUrl,foodName,foodDetail",
      previewUrl,
      foodName,
      foodDetail,
    );
  ...
  submitFoodDetailForUpdate() {
    if (this.mode === "photo") {
      const data = {
        ...
        type: "V",
        calories:
          this.foodDetail?.macros?.calories === undefined
            ? 0
            : this.foodDetail.macros?.calories,
        protein:
          this.foodDetail?.macros?.protein === undefined
            ? 0
            : this.foodDetail.macros?.protein,
        fat:
          this.foodDetail?.macros?.fats === undefined
            ? 0
            : this.foodDetail.macros?.fats,
        carbs:
          this.foodDetail?.macros?.carbs === undefined
            ? 0
            : this.foodDetail.macros?.carbs,
        fiber:
          this.foodDetail?.macros?.fiber === undefined
            ? 0
            : this.foodDetail.macros?.fiber,
        slot: this.slot,
        portionUnit: this.unit,
        portionQuantity: this.portion,
        score:
          this.foodDetail?.macros?.nutri_score === undefined
            ? 0
            : this.foodDetail?.macros?.nutri_score,
  ...
  fromBarcodeUpdateFoodDetail(foodName, foodDetail) {
    ...
    const data = {
      ...
      type: "V",
      calories:
        foodDetail?.macros?.calories === undefined
          ? 0
          : foodDetail.macros?.calories,
      protein:
        foodDetail?.macros?.protein === undefined
          ? 0
          : foodDetail.macros?.protein,
      fat: foodDetail?.macros?.fats === undefined ? 0 : foodDetail.macros?.fats,
      carbs:
        foodDetail?.macros?.carbs === undefined ? 0 : foodDetail.macros?.carbs,
      fiber:
        foodDetail?.macros?.fiber === undefined ? 0 : foodDetail.macros?.fiber,
      slot: this.slot,
      portionUnit: this.unit,
      portionQuantity: this.portion,
  ...
  updateFoodDetailPraveenapi(data) {
    this.appServices.updateFoodDetailPraveenApi(data).then(
      (res: any) => {
        ...
        this.openForBarcode(
          this.previewUrl,
          this.foodDetailForBarcode?.food,
          this.foodDetailForBarcode,
        );
    ...
    this.appServices.barcodeFootnoteImageSend(itemNumber).subscribe(
      (res: any) => {
        ...
        if (res?.product_name !== undefined) {
          ...
          // this.stopCamera();
          this.openNutritionModelBarCode(
            this.previewUrl,
            res?.product_name,
            this.barcodeFoodDetail,
          );
        } else {
          this.utilities.hideLoader();
