...
export class MealWorkoutPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.data.data.forEach((elm) => {
      ...
      if (elm.Score === 1) {
        elm.option = "Below average";
      }
      if (elm.Score === 0) {
        elm.option = "Bad";
      }
      if (elm.Score === -1) {
        elm.option = "Not recommended";
      }
  ...
  async changed(data) {
    console.log("data111111", data,this.index);
  ...
  async loogeAction(event, d, i) {
    ...
    if (data?.data) {
      if (data?.data?.type === "track") {
        data.data.updatedData.Calories = (data?.data?.updatedData?.Calories/data?.data?.updatedData?.portion);
        ...
        // this login only for manage calories from dietplan. total calories coming from DB on this page.
        data.data.updatedData.Calories = (data?.data?.updatedData?.Calories/data?.data?.updatedData?.portion);
        this.addCal(data?.data?.updatedData, i);
        ...
        foodCodeList: [
          {
            ...
            portion: 0,
            eaten: eaten,
            foodSource: item?.foodSource?.toLowerCase(),
          },
        ],
        isUpdateDiet: true,
      };
      console.log(item, eaten, status);
      //       foodSource
      // :
      // "internal"
  ...
  async eatenStatusUpdate(item, eaten, status) {
    if (this.currentDateIndex == 0) {
      ...
      console.log(item, eaten, status);
      // if(item?.foodSource === "external"){
      //   datas.foodCodeList.push({foodSource:'external'})
      // }
      //       foodSource
      // :
      // "external"
